...
class MetadataWidget(ipw.VBox):
    def __init__(self):
        ...
        self.process_description = ipw.Text(
        ...
        children = [
            self.process_description, self.num_machines,
            self.num_mpiprocs_per_machine, self.num_cores_per_mpiproc,
            ipw.HBox([
                ipw.HTML("walltime:"), self.walltime_d, self.walltime_h,
                self.walltime_m
            ])
        ]
        ...
        return {
            ...
            "options": {
                "resources": {
                    ...
                    self.num_mpiprocs_per_machine.value,
                    "num_cores_per_mpiproc": self.num_cores_per_mpiproc.value,
                },
                "max_wallclock_seconds":
                int(self.walltime_d.value * 3600 * 24 +
                    self.walltime_h.value * 3600 + self.walltime_m.value * 60),
                'withmpi':
                True,
            }
        }
